...
import pandas as pd
string_data = []
fpcap1 = open("/Users/hopefulguowei/PycharmProjects/database/undo/attack/15GOOSE.pcap", 'rb')  # 读协议数据包
A=[[]for i in range(30)]
...
while (i < len(string_data)):
    ...
    if(string_data[i+19]=='\x01' and string_data[i+21]=='\x1a'): # 智能终端状态数据包信息
        ...
        A[0].append(time3)
        print(A[0][-1])
        A[1].append(time3-IT_STATE_TIME)
        IT_STATE_TIME=time3
        ...
        k=k+1
        j=j+1
        if (string_data[i + 27] == '\x4d'):
            A[28].append(0)
            A[29].append(0)
        else:
            A[28].append(1)
            A[29].append(struct.unpack('B', string_data[i + 27])[0]-6)
        i = i + packet_len + 16
        ...
        j=0
        i = i + packet_len + 16
    elif (string_data[i+19] == '\x01' and string_data[i+21] == '\x14'):
...
dataframe.to_csv("/Users/hopefulguowei/PycharmProjects/database/doneData/attack/15GOOSE.csv",index=False,sep=',')