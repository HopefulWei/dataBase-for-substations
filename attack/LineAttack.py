...
import pandas as pd
string_data = []
fpcap1 = open("/Users/hopefulguowei/PycharmProjects/database/undo/attack/2GOOSE.pcap", 'rb')  # 读协议数据包
A=[[]for i in range(30)]
...
while (i < len(string_data)):
    ...
    if(string_data[i+19]=='\x01' and string_data[i+21]=='\x14'): # 智能终端状态数据包信息6
        time2 = struct.unpack('<I', string_data[i:i + 4])[0]
        A[0].append(time2)
        A[19].append(time2 - LPD_TIME)
        k=k+1
        j=j+1
        if (string_data[i + 27] == '\x01'):
            ...
            A[20].append(struct.unpack('>I', string_data[i + 0x7f + 4:i + 0x83+ 4])[0])
            A[21].append(struct.unpack('>I', string_data[i + 0x89+ 4:i + 0x8d+ 4])[0])
            A[22].append(struct.unpack('>I', string_data[i + 0x8f+ 4:i + 0x93+ 4])[0])
            A[23].append(struct.unpack('B', string_data[i + 0x95+ 4])[0])
            A[24].append(struct.unpack('>I', string_data[i + 0x98+ 4:i + 0x9c+ 4])[0])
            A[25].append(struct.unpack('B', string_data[i + 0xab+ 4])[0])
            A[26].append(struct.unpack('B', string_data[i + 0xae+ 4])[0])
            A[27].append(struct.unpack('B', string_data[i + 0xb1+ 4])[0])
            LPD_TIME = time2
            A[28].append(1)
            A[29].append(struct.unpack('B', string_data[i + 27])[0])
        i = i + packet_len + 16

    elif (string_data[i+19] == '\x01' and string_data[i+21] == '\x1a'):
        ...
        j=0
        i = i + packet_len + 16
    elif (string_data[i+19] == '\x01' and string_data[i+21] == '\x18'):
        time3 = struct.unpack('<I', string_data[i:i + 4])[0]
        for m in range(k):
            A[11].append(time3 - IT_ALARM_TIME)
...
dataframe.to_csv("/Users/hopefulguowei/PycharmProjects/database/doneData/attack/02GOOSE.csv", index=False, sep=',')